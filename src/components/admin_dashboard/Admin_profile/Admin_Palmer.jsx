...
import {Timestamp, collection, onSnapshot, getDocs, deleteDoc, doc, addDoc, getDoc, query, orderBy, where, setDoc } from "firebase/firestore";
  ...
  const fetchArchivedAppointments = async () => {
    ...
    try {
      const archivedRef = query(
        ...
        where("lawyer.name", "==", PALMER_NAME) // 🔹 Filter for Evasco only
      );
